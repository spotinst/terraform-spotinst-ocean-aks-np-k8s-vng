...
| <a name="input_autoscale_auto_headroom_percentage"></a> [autoscale_auto_headroom_percentage](#input_autoscale_auto_headroom_percentage) | Optionally set a number between `0-200` to control the percentage of VNG resources dedicated to automatic headroom. | `number` | `null` | no |
...
| <a name="input_vm_max_map_count"></a> [vm\_max\_map\_count](#input\vm\_max\_map\_count)                                                                        | Maximum number of memory map areas a process may have. Can be configured only if OS type is Linux.                                                                                                                                                                                                                                                                                                               | `number`        | `null` |    no    |
...
| <a name="input_draining_timeout"></a> [draining\_timeout](#input\_draining\_timeout) | Time in seconds to allow the node to drain before it is terminated. Valid values are 300-3600. | `number` | `null` | no |
| <a name="input_should_utilize_commitments"></a> [should\_utilize\_commitments](#input\_should\_utilize\_commitments) | Determines whether to utilize any existing Azure Savings Plans or Reserved Instances associated with the subscription for On-Demand VMs. | `bool` | `false` |    no    |
| <a name="input_restrict_scale_down"></a> [restrict\_scale\_down](#input\_restrict\_scale\_down) | When set to true, nodes in this VNG will be protected from scale-down as long as they have reschedulable workloads running. | `bool` | `false` |    no    |
...
| <a name="input_time_windows"></a> [time\_windows](#input\_scheduling\_shutdown\_hours\_time\_windows)            | The times that the shutdown hours will apply.    | ` list(string)` | `null`  |    no    |
| <a name="input_is_enabled"></a> [is\_enabled](#input\_scheduling\_shutdown\_hours\_is\_enabled)                  | Flag to enable or disable the shutdown hours mechanism. When False, the mechanism is deactivated, and the cluster remains in its current state.  | `bool`  | `null`  |    no    |
